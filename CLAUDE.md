...
| `Regex._use_onepass` (captures, one-pass NFA — at most one thread consumes each byte — with a loop the backtracker would recurse through, `_sbt_general_loop`) | One-pass DFA (`onepass.mojo`): `match()` is one forward table walk writing the slots; the capture lane below uses it for the anchored attempt and the span confirm instead of the backtracker. Simple-loop shapes stay on the backtracker (faster on short inputs) |
