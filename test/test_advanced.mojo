...
def test_pos_lookahead_no_match() raises:
    var re = compile("foo(?=bar)")
    assert_false(re.search("foobaz").matched, msg="foo(?=bar) should not match 'foobaz'")
...
def test_neg_lookahead() raises:
    var re = compile("foo(?!bar)")
    assert_true(re.search("foobaz").matched, msg="foo(?!bar) should match 'foobaz'")
    assert_false(re.search("foobar").matched, msg="foo(?!bar) should not match 'foobar'")
...
def test_pos_lookbehind_no_match() raises:
    var re = compile("(?<=foo)bar")
    assert_false(re.search("bazbar").matched, msg="(?<=foo)bar should not match 'bazbar'")
...
def test_neg_lookbehind() raises:
    var re = compile("(?<!foo)bar")
    assert_true(re.search("bazbar").matched, msg="(?<!foo)bar should match in 'bazbar'")
...
def test_backref_basic() raises:
    ...
    assert_false(re.match("aaba").matched, msg="(a+)b\\1 should not match 'aaba'")
