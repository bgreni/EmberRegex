...
def test_anchor_bol() raises:
    ...
    assert_false(re.search("xabc").matched, msg="^abc should not match in middle")

def test_anchor_eol() raises:
    var re = compile("abc$")
    assert_true(re.match("abc").matched)
    assert_true(re.search("abc").matched)
    assert_false(re.search("abcx").matched, msg="abc$ should not match with trailing")
...
def test_word_boundary_basic() raises:
    ...
    var result = re.search(input)
    assert_true(result.matched, msg="\\bword\\b should match 'word' in 'a word here'")
    assert_equal(result.start, 2)
    assert_equal(result.end, 6)

def test_word_boundary_no_match() raises:
    var re = compile("\\bword\\b")
    assert_false(re.search("sword").matched, msg="\\bword\\b should not match in 'sword'")
    assert_false(re.search("wordy").matched, msg="\\bword\\b should not match in 'wordy'")
...
def test_not_word_boundary() raises:
    ...
    assert_true(re.search("sword").matched, msg="\\Bword should match in 'sword'")
    assert_false(re.search("word").matched, msg="\\Bword should not match at start")
